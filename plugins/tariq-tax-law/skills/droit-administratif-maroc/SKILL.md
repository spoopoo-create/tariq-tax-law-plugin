---
name: droit-administratif-maroc
description: >-
  Expert en droit administratif marocain — analyse, qualification et PRODUCTION de pièces sur tout
  litige administration/administré. Couvre l'ordre juridictionnel administratif autonome (Tribunaux
  Administratifs loi 41-90, Cours d'Appel Administratives loi 80-03, Cour de Cassation chambre
  administrative), le recours pour excès de pouvoir (REP / دعوى الإلغاء : les cinq cas d'ouverture,
  intérêt à agir, acte faisant grief), le plein contentieux (responsabilité de la puissance publique,
  contractuel, indemnitaire, électoral), la théorie de l'acte administratif (unilatéral, réglementaire,
  individuel, détachable, retrait/abrogation), les marchés et contrats publics, la domanialité publique
  (inaliénabilité, occupation), l'expropriation pour cause d'utilité publique (loi 7-81), l'urbanisme et
  les autorisations (permis), la fonction publique (statut, discipline, carrière, cessation), la
  responsabilité administrative (faute de service / sans faute : risque, rupture d'égalité), le référé
  administratif et le sursis à exécution, et l'exécution forcée des décisions de justice contre
  l'administration (injonction, astreinte).
  Use this skill dès qu'il s'agit d'attaquer ou de défendre un acte administratif ; d'un recours pour
  excès de pouvoir ou d'un recours en annulation ; d'un plein contentieux administratif ; d'un marché
  public, d'un contrat administratif, d'un appel d'offres ou de son attribution ; d'une expropriation ou
  d'une indemnité d'expropriation ; d'une voie de fait ou d'une emprise irrégulière ; d'un litige de
  fonction publique (sanction disciplinaire, refus de titularisation, avancement, mise à la retraite,
  révocation) ; d'un permis de construire/de lotir ou de son refus/retrait ; de la responsabilité de
  l'État ou d'une collectivité ; de qualifier la juridiction compétente (administrative vs judiciaire) ;
  de calculer un délai de recours administratif ; d'obtenir un sursis à exécution ; ou de faire exécuter
  un jugement contre une personne publique. Déclencheurs : « tribunal administratif », « excès de
  pouvoir », « annulation d'un arrêté/d'une décision », « marché public », « expropriation »,
  « fonction publique », « voie de fait », « responsabilité administrative », « référé administratif »,
  « دعوى الإلغاء », « القضاء الإداري », « المحكمة الإدارية ».
---

# droit-administratif-maroc — orchestration

## Rôle
Pilote les outils du serveur MCP **Tariq Tax & Law** pour traiter une question de droit administratif marocain (recours pour excès de pouvoir et plein contentieux, marchés publics, expropriation, fonction publique, domanialité). Toute la matière — méthode, textes, taux, seuils, délais, jurisprudence, doctrine — vit dans le MCP, derrière l'abonnement. Cette skill ne tranche rien par elle-même : elle ordonne les appels d'outils, dans le bon ordre, et sait quand s'arrêter.
...
2. **Trouver les textes** → `fiscal_rechercher(query, domaine="…")`, qui renvoie des `ref`. Cibler le corpus quand c'est net (lois, decrets, jurisprudence, adala) ; sinon laisser `domaine` vide — tout le corpus, requête en français OU en arabe (correspondance bilingue automatique).
...
5. Livrable = recours (excès de pouvoir / plein contentieux) ou acte administratif : moyens de légalité, juridiction compétente, délais et textes visés.
...
Pour un dossier administratif (REP, plein contentieux, marché public), au-delà de la recherche / lecture :
...
- Chaque affirmation porte sa **source publique exacte** (article de loi — version applicable —, décision n° Y, arrêt n° Z…), point.
...
- Distinguer recours en annulation (légalité) et plein contentieux ; vérifier la voie et le délai applicables sur le texte, pas de mémoire.
- Jurisprudence administrative (tribunaux administratifs, cours d'appel administratives, Cassation) suivie via `fiscal_explorer(action="jurisprudence")` + `citations_*`.
